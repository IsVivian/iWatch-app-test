...
class Flight {
  ...
  var shortNumber: String {
    get {
        return number.substring(from: number.index(number.startIndex, offsetBy: 2))
  ...
  class func allFlights() -> [Flight] {
    ...
    if let path = Bundle.main.path(forResource: "Flights", ofType: "json"), let data = NSData(contentsOfFile: path) {
      do {
  ...
  init(origin: String, destination: String, number: String, boardsAt: String, delayed: String, gate: String, seat: String) {
    ...
    let number = dictionary["number"]!
        let boardsAt = formatter.string(from: NSDate().addingTimeInterval(Double(arc4random_uniform(21600) + 1800)) as Date)
